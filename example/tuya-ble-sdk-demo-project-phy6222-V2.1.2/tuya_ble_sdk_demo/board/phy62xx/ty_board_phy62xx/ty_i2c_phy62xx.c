...
#define TY_I2C_PIN_SDA  P26
#define TY_I2C_PIN_SCL  P31
...
static void ty_i2c_scan_address(void)
{
    ...
    for (address = 1; address <= 127; address++) { // tbd
       uint32_t ret = hal_i2c_read(AP_I2C1, address, 0x40, &sample_data, sizeof(sample_data)); //0x40
...
uint32_t ty_i2c_init(void)
{
    ...
    hal_i2c_init(I2C_1, I2C_CLOCK_400K);  // 100k std clk speed
    //ty_i2c_scan_address();	
    return 0;
}
...
//		nrf_gpio_cfg_input(TY_I2C_PIN_SDA, NRF_GPIO_PIN_PULLUP);
		hal_gpio_pin_init(TY_I2C_PIN_SDA, IE);
		hal_gpio_pull_set(TY_I2C_PIN_SDA, WEAK_PULL_UP);

	} else if(mode == GPIO_OUTPUT_MODE){
//        nrf_gpio_cfg_output(TY_I2C_PIN_SDA);
		hal_gpio_pin_init(TY_I2C_PIN_SDA,OEN);
	}
}
...
{
	hal_gpio_pin_init(TY_I2C_PIN_SDA,OEN);
  	hal_gpio_pin_init(TY_I2C_PIN_SCL,OEN);
//	nrf_gpio_cfg_output(TY_I2C_PIN_SDA);

...
/* network peripherals */
#define NET_KEY_PIN                 GPIO_P33
#define NET_LED_PIN                 GPIO_P14 //配网灯，无
...
STATIC VOID_T __wait_bind_timer_cb(VOID_T);//声明局部变量
#if TUYA_BLE_BEACON_KEY_ENABLE
 ...
 * @return none
 */
VOID_T __net_key_init(VOID_T)//ok
{
    KEY_RET ret;

    sg_key_def_s.port = NET_KEY_PIN;
    sg_key_def_s.active_low = TRUE;//1
    ...
    if (KEY_OK != ret) {
		...
		//TUYA_APP_LOG_DEBUG("__net_key_init: %d",sg_key_def_s.active_low);
}
 ...
 * @return none
 */
VOID_T __net_led_init(VOID_T)//ok
{
	  //TUYA_APP_LOG_DEBUG("__net_led_init0");
    LED_RET ret = tuya_create_led_handle(NET_LED_PIN, TRUE, &sg_net_led_handle);//0 反向的，高电平，关灯
    if (LED_OK != ret) {
        TUYA_APP_LOG_ERROR("Network led init err:%d.", ret);
    }
		//TUYA_APP_LOG_DEBUG("__net_led_init1：%d.",ret);
}
 ...
 * @return none
 */
STATIC VOID_T __set_net_led_status(NET_LED_STAT status)//ok
{
 ...
 * @return none
 */
VOID_T tuya_net_proc_init(VOID_T)//ok
{
	  //TUYA_APP_LOG_DEBUG("tuya_net_proc_init0");
    ...
    if ((ble_conn_sta == BONDING_UNCONN) ||
        ...
        F_BLE_BOUND = SET;
        F_WAIT_BINDING = CLR;
        __set_net_led_status(NET_LED_OFF);//ok
    } else {
        F_BLE_BOUND = CLR;
        F_WAIT_BINDING = SET;
        __set_net_led_status(NET_LED_FLASH_QUICK);//ok
        tuya_ble_timer_start(wait_bind_timer);
    }
		//TUYA_APP_LOG_DEBUG("tuya_net_proc_init1");
}
...
VOID_T __allow_binding(VOID_T)
{
    ...
    tuya_ble_timer_start(wait_bind_timer);
		//TUYA_APP_LOG_DEBUG("__allow_binding");
}
...
VOID_T __set_net_led_power(VOID_T)
{
    ...
    if (NET_LED_OFF == sg_net_led_status) {
        __set_net_led_status(NET_LED_ON);
			  //TUYA_APP_LOG_DEBUG("NET_LED_ON");
    } else {
        __set_net_led_status(NET_LED_OFF);
			//TUYA_APP_LOG_DEBUG("NET_LED_OFF");
    }
		//TUYA_APP_LOG_DEBUG("__set_net_led_power");
}
...
STATIC VOID_T __net_key_cb(KEY_PRESS_TYPE_E type)
{
    ...
    default:
		...
		//TUYA_APP_LOG_DEBUG("__net_key_cb");
}

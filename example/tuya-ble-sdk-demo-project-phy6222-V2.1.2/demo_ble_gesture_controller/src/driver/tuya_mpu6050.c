 ...
 * @param[in] clk: clock source时钟源
 * @param[in] g_fsr: gyroscope's full-scale range陀螺仪的满标度范围
 * @param[in] a_fsr: accelerometer's full-scale range加速计的满标度范围
 * @param[in] smp_rt: sample rate采样率
 * @param[in] pin: interrupt pin中断口
 * @param[in] type: interrupt type中断类型
 * @param[in] int_cb: interrupt callback function中断回调函数
 * @return operation result
 */
MPU_RET tuya_mpu6050_init(_IN CONST MPU_CLK_E clk, _IN CONST MPU_GYRO_FSR_E g_fsr, _IN CONST MPU_ACCEL_FSR_E a_fsr,
                          _IN CONST USHORT_T smp_rt, _IN CONST gpio_pin_e pin, _IN CONST TY_GPIO_IRQ_TYPE_E type,
                          _IN TY_GPIO_IRQ_CB int_cb)
{
    ...
    /*
    if (int_cb != NULL) {
        //if (tuya_gpio_irq_init(pin, type, int_cb)) {//配置模组外部中断引脚
        //    return MPU_ERR_IRQ_INIT_FAILED;
        //}
        if (TY_GPIO_IRQ_FALLING == type) {
            __mpu6050_set_int(TRUE);//配置6050int脚，平时拉高，数据就绪翻转为低
        } else {
            __mpu6050_set_int(FALSE);//配置6050int脚，平时拉低，数据就绪翻转为高
        }
    }
		*/
    return MPU_OK;
...
/**
 * @brief read accelerometer data from MPU6050 (specified unit)从MPU6050（指定单位）读取加速计数据
...
/**
 * @brief read gyroscope data from MPU6050 (specified unit)从MPU6050读取陀螺仪数据（指定单位）
...
/**
 * @brief MPU6050 built-in DMP init MPU6050内置DMP初始化
 * @param[in] g_fsr: gyroscope's full-scale range 陀螺仪的满标度范围
 * @param[in] a_fsr: accelerometer's full-scale range 加速度计的满标度范围
 * @param[in] pin: interrupt pin 中断引脚
 * @param[in] type: interrupt type 中断类型
 * @param[in] int_cb: interrupt callback function 中断回调函数
 * @return MPU_RET
 */
MPU_RET tuya_mpu6050_dmp_init(_IN CONST MPU_GYRO_FSR_E g_fsr, _IN CONST MPU_ACCEL_FSR_E a_fsr,
                              _IN CONST gpio_pin_e pin, _IN CONST TY_GPIO_IRQ_TYPE_E type, _IN TY_GPIO_IRQ_CB int_cb)
{
    /* I2C init */
...
/**
 * @brief read data of MPU6050 built-in DMP (specified unit)读取MPU6050内置DMP（指定单元）的数据
 * @param[in] g_unit: gyroscope unit陀螺仪装置
 * @param[out] gyro: gyroscope data of 3-axis三轴陀螺仪数据
 * @param[in] a_unit: accelerometer unit加速度计单元
 * @param[out] accel: accelerometer data of 3-axis三轴加速度计数据
 * @param[out] roll: the angle rotated around the X-axis绕X轴旋转的角度
 * @param[out] pitch: the angle rotated around the Y-axis绕Y轴旋转的角度
 * @param[out] yaw: the angle rotated around the Z-axis绕Z轴旋转的角度
 * @return 0 means success
 */

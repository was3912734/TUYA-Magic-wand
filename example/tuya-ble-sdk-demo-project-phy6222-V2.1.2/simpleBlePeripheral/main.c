...
static void hal_low_power_io_init(void)
{
    ...
    ioinit_cfg_t ioInit[]=
    {
        ...
        {GPIO_P07,   GPIO_FLOATING  },//ct pow 6050上电
...
{
		//hal_gpio_pin2pin3_control(GPIO_P02, 1);
	  //hal_gpio_pin2pin3_control(GPIO_P03, 1);
	  //hal_gpio_pin2pin3_control(GPIO_P32, 1);
	  //hal_gpio_pin2pin3_control(GPIO_P33, 1);
	  //hal_gpio_pin2pin3_control(GPIO_P34, 1);
	  //hal_gpio_cfg_analog_io(GPIO_P11, Bit_DISABLE);	
	  //hal_gpio_pin_init(GPIO_P02,GPIO_INPUT);
	  //hal_gpio_pin_init(GPIO_P03,GPIO_INPUT);
	  hal_gpio_pin_init(GPIO_P11,GPIO_INPUT);
	  hal_gpio_pin_init(GPIO_P33,GPIO_INPUT);
	  hal_gpio_pin_init(GPIO_P34,GPIO_INPUT);
    //tuya_gpio_init(GPIO_P32,FALSE,TRUE);
	  //ty_pin_init(GPIO_P11,TY_PIN_MODE_IN_IRQ_RISE_FALL);
	
    hal_low_power_io_init();
    clk_init(g_system_clk); //system init
    hal_rtc_clock_config((CLK32K_e)g_clk32K_config);
    hal_pwrmgr_init();
    //hal_gpio_wakeup_set(GPIO_P02,NEGEDGE);
    hal_pwrmgr_register(MOD_USR2, ty_sleep_handler, ty_wakeup_handler);//设置睡眠回调函数
    ty_system_exit_sleep();//使能
    ...
    hal_gpio_init();
		...
		//tuya_gpio_init(GPIO_P02,TRUE,TRUE);
		//tuya_gpio_init(GPIO_P03,TRUE,TRUE);
		//tuya_gpio_init(TY_GPIO_32,TRUE,TRUE);
		//tuya_gpio_init(GPIO_P33,TRUE,TRUE);
		//tuya_gpio_init(GPIO_P34,TRUE,TRUE);
